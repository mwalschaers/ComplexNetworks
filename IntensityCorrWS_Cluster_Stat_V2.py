...
def gaussianCorrelator(i,j):
    global adj
    global squeezing
    s = squeezing
    m = np.size(adj[0])
    Aij = adj[i,j]
    A2ij = np.matmul(adj,adj)[i,j]
    One = np.eye(m)
    degree= np.sum(adj,axis=1)
    if i != j :
        result = s**2 * (A2ij**2 + 2*Aij)/8.

    else:
        result = (s**2 + 1./(s**2) + s**2 * degree[i]**2 + 2.*degree[i] - 2.)/8.
...
def makeCorrelationMatrixSingleModeSub(n,g):
    ...
    for i in range(int(m/2)):
        for j in range(int(i+1)):
            ...
            GaussianTesti = One[i,subindex] + adj[i,subindex] + A2[i,subindex]
            GaussianTestj = One[j,subindex] + adj[j,subindex] + A2[j,subindex]
            GaussianTest = min(GaussianTesti,GaussianTestj)

            if GaussianTest != 0 :
                if i!=j :
                    cx = singleModeSubCorrelator(f,g,n)
                    correlationMatrix[i,j] = np.real(cx)
                    correlationMatrix[j,i] = np.real(cx)
                else:
                    cx = singleModeSubCorrelatorDiag(f,g,n)
                    correlationMatrix[i,i] = np.real(cx)


def makeGaussianCorrelationMatrixSingleModeSub(n):

    #global correlationMatrix
    global adj
    global squeezing
    s = squeezing
    #m = np.size(correlationMatrix[0])
    A2 = np.matmul(adj,adj)
    #Cmatrix= np.empty([int(m/2.),int(m/2.)])
    degree= np.sum(adj,axis=1)
    result = s**2 * (A2**2 + 2*adj)/8.
    diag = (s**2 + 1./(s**2) + s**2 * degree**2 + 2.*degree - 2.)/8.
...
#        for j in range(int(i+1)):
#
#            cx = gaussianCorrelator(i,j)
#            correlationMatrix[i,j] = cx
#            correlationMatrix[j,i] = cx
...
f.write('\n')
f.write('We created a Watts-Strogatz graph state, with the following properties:')
f.write('Size of the network: ' + str(n) +'\n')
f.write('Rewiring distance: ' + str(m)+'\n')
f.write('Rewiring probability: ' + str(p)+'\n')
f.write('Dimension of the initial regular graph: ' + str(dim)+'\n')
...
for rep in range(repetitions):
    ...
    connected = False
    while not connected:
        g = graph.Graph.Watts_Strogatz(dim=dim, size=n, nei=m, p=p)
        connected = g.is_connected()

    ad = g.get_adjacency()
    adj = np.array(ad.data)

    squeeze=SQ*np.ones(np.size(adj[0]))
    ...
    #correlationMatrix = np.empty([int(M/2),int(M/2)])

    if rand == 1:
        randomVertex = np.random.randint(n)
        subtractionMode = np.array(One[randomVertex].tolist()+Zero[0].tolist())
        subindex = randomVertex
    else:
        subtractionMode = np.array(One[maxDegVert].tolist()+Zero[0].tolist())
        subindex = maxDegVert

    for nSubtractions in [0,10]:

        if nSubtractions == 0 :
            correlationMatrix = makeGaussianCorrelationMatrixSingleModeSub(n)
        else:
            makeCorrelationMatrixSingleModeSub(nSubtractions,subtractionMode)
